...
///
/// ملاحظة تصميم: الشاشات ما زالت تعدّل model.GymDB في الميموري ثم تنادي save(),
...
class AppDatabase extends _$AppDatabase {
  ...
  Future<model.GymDB> loadAll() async {
    ...
    for (final s in sessionRows) {
      ...
      workoutSessions.add(model.WorkoutSession(
        id: s.id,
        dayId: s.dayId,
        dayName: s.dayName,
        date: s.date,
    ...
    final photoList = photoRows
        .map((p0) => model.ProgressPhoto(id: p0.id, date: p0.date, path: p0.path))
        .toList();
  ...
  String _daySnapshot(model.WorkoutDay d, int sortOrder) => _encode({
        'id': d.id,
        'name': d.name,
        'sortOrder': sortOrder,
      });

  String _exerciseSnapshot(model.ExerciseDef e, int sortOrder) => _encode({
        ...e.toJson(),
        'sortOrder': sortOrder,
      });

  String _settingsSnapshotOf(model.AppSettings s) => _encode(s.toJson());

  String _inbodySnapshot(model.InBodyEntry e) => _encode(e.toJson());

  String _photoSnapshot(model.ProgressPhoto p) => _encode(p.toJson());

  String _sessionSnapshot(model.WorkoutSession s) => _encode(s.toJson());

  void _captureSnapshots(model.GymDB db) {
    _daySnapshots = {
  ...
  Future<void> saveAll(model.GymDB db) async {
  ...
  Future<void> saveSettings(model.AppSettings s) async {
    await transaction(() async {
      await into(appSettingsRow).insertOnConflictUpdate(
        AppSettingsRowCompanion.insert(
          id: const Value(0),
